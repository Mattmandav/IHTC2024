  ...
  : weights(SOFT_COST_COMPONENTS) 
{
  ...
  nlohmann::json j_f, j_p, j_r, j_s, j_ot, j_n;
  nlohmann::json j_in = nlohmann::json::parse(file_name);
...
{
	nlohmann::json j_p, j_n;
	unsigned i, j, p, n, cn;
	int d, s, r, t;
	string patient_id, nurse_id, room_id, ot_id, shift_name;
	nlohmann::json j_sol = nlohmann::json::parse(file_name);
	Reset();
...
int main(int argc, const char *argv[])
{
  if (argc != 3 && argc != 4)
  {
    cerr << "Usage: " << argv[0] << " input_json_string output_json_string [verbose]" << endl;
    exit(1);
  }
